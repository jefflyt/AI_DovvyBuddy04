...
- **LLM integration:** `apps/web/src/shared/lib/model-provider` implements a `ModelProvider` switch (`LLM_PROVIDER=groq|gemini`)
- **RAG helpers:** `apps/web/src/shared/lib/rag` contains retrieval client helpers and orchestration bindings
- **Key files:** `package.json`, `pnpm-lock.yaml`, `drizzle.config.ts`, `apps/web/src/shared/lib/`

## 4. Python Backend (FastAPI services)

- **Framework:** FastAPI (async)
- **ORM / Migrations:** SQLAlchemy 2.0 (async) + Alembic
- **Services:** embeddings, LLM wrappers, RAG pipeline components under `apps/api/app/services/`
- **Packaging:** `pyproject.toml` (setuptools); prefer virtualenv or poetry for local dev
- **Key files:** `apps/api/app/`, `apps/api/pyproject.toml`, `apps/api/openapi.yaml`
...
- **Orchestration:** ADK / Genkit integration code lives under `apps/web/src/shared/lib/agent/` for multi-agent flows
- **Env vars (examples):** `GEMINI_API_KEY`, `GROQ_API_KEY`, `ENABLE_ADK`, `ADK_MODEL`

## 7. RAG Pipeline

- **Chunking:** markdown-aware chunker in Python RAG services (`apps/api/app/services/rag/chunker.py`)
- **Retriever:** vector search using `pgvector` (`apps/api/app/services/rag/retriever.py`)
- **Indexing:** ingest → chunk → embed → store vectors in Postgres (or object-storage-backed index if chosen)
- **Orchestration:** TypeScript orchestrator (`apps/web/src/shared/lib/orchestration/*`) coordinates retrieval + safety + model calls
...
- **TS logging:** `pino`
- **Python logging:** stdlib `logging` configured in `apps/api/app/core`
...
- **Architecture & PR plans:** `docs/archive/plans/` (see PR3, PR3.1, PR3.2\*)
- **Service docs & verification:** `apps/api/README_SERVICES.md`, `apps/api/VERIFICATION_SUMMARY_PR3.2b.md`
